defmodule JsonParserTest do
  ...
  test "unicode_escape_parser" do
    assert JsonParser.Grammar.unicode_escape_parser().("\\u0041rest") == {:ok, "A", "rest"}
    assert {:fail, _, _} = JsonParser.Grammar.unicode_escape_parser().("\\uFFFZ abcd")
    assert {:fail, _, _} = JsonParser.Grammar.unicode_escape_parser().("\\u004")
  end
